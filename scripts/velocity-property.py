...
def create_visualizer_cube(name):
...
# Function to update the cubes' positions
def update_visualizer():
    curve = bpy.data.objects.get("track-curve")  # Change to your curve's name
    cube = create_visualizer_cube("Traveled_Distance_Cube")  # Get or create the visualizer cube
    lagged_cube = create_visualizer_cube("Traveled_Distance_Cube_Lagged")  # Second cube
    ...
    if curve and curve.data:
        ...
        if "traveled_distance_lagged" not in curve.data:
            curve.data["traveled_distance_lagged"] = 0.0
        
        # Update current frame traveled distance
        curve.data["traveled_distance"] = traveled_distance
        
        # Move the first cube along the X-axis
        cube.location.x = traveled_distance
        
        # Retrieve previous frame's traveled distance and update the lagged cube
        curve.data["traveled_distance_lagged"] = curve.data.get("traveled_distance_lagged", traveled_distance)
        lagged_cube.location.x = curve.data["traveled_distance_lagged"]
        
        # Store current traveled distance for next frame's lagged update
        curve.data["traveled_distance_lagged"] = traveled_distance
